...
module.exports = function(server) {
  server.post({
    ...
    return req.model.create(req.body, function(err) {
      return res.send({
        status: 'ok',
  ...
  server.post('/account/key/:key-name', function(req, res, next) {
    var children, endpoints, key, keyname, parent, reg, regex, updated, _base, _i, _j, _k, _len, _len1, _len2;
    keyname = req.params['key-name'];
    if ((_base = req.body).parent == null) {
      _base.parent = req.account.data.keys[keyname].parent || req.key.name;
    }
    if ((req.account.data.keys[keyname] != null) && (req.body.refresh == null)) {
      ...
      return req.account.save(function(err) {
        if (err) {
          throw err;
        }
        return res.send({
      ...
      return req.account.generateKey(keyname, req.body.parent, req.body.endpoints, function(err, key) {
        return res.send({
          status: 'ok',
