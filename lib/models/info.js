    ...
    return this.table.addStreamOperationType('info_multiplex', {
      ...
      exec: function(row, callback) {
        var handler_cache, interpolate, modes, time,
          _this = this;
        handler_cache = this.modules.cache.create('info-handlers-' + this.account, true, function(key, next) {
          return _this.stream.db.collection('info_handlers').find().toArray(next);
        });
        interpolate = function(str, fact, info) {
          var context, section, sections, _i, _len;
          context = {
            fact: fact,
            info: info
          };
        ...
        return handler_cache.get(function(err, handlers, cache_hit) {
          ...
          return _this.modules.async.map(handlers, iterator, function(err, rows) {
