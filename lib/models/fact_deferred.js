...
var Cache, DeferredObject, Fact_deferred_Model, Model, async, wrapArray,
...
module.exports = Fact_deferred_Model = (function(_super) {
  ...
  Fact_deferred_Model.prototype.markUpdated = function(callback) {
    if (this.data._id) {
      return this.db.collection('fact_updates').insert({
        type: this.type,
        id: this.data._id,
        time: +(new Date)
      }, callback);
  ...
  Fact_deferred_Model.prototype.markUpdatedFull = function(callback) {
    ...
    return this.table.aggregate({
      ...
      return async.map(ids, insert, function(err, result) {
        var id;
        id = (result || []).filter(function(v) {
          var _ref;
          return (v != null ? (_ref = v[0]) != null ? _ref.id : void 0 : void 0) != null;
        }).map(function(v) {
          return v[0].id;
        });
  ...
  Fact_deferred_Model.prototype["import"] = function(data, callback) {
    this.data = data || {};
  ...
  Fact_deferred_Model.prototype.withMap = function(_with, map, shim, callback) {
    var args,
      _this = this;
    ...
    map = map || {};
    return async.map(_with, this.data.get.bind(this.data), function() {
      var next;
      if (!map) {
      ...
      return next(function() {
        ...
        get = function(arg, next) {
          ...
          return _this.data["eval"](path, function(err, result) {
            return next(null, obj[key] = result || def);
          });
        };
        maps = (function() {
          var _results;
        ...
        if (maps.length > 0) {
          ...
          return async.map(maps, get, function() {
