...
(function() {
  ...
  module.exports = function(job, done) {
    var account, accountID, fns, row, s, t, time,
      _this = this;
    account = null;
    ...
    fns.setup = function(next) {
      ...
      if (account.conditions == null) {
        account.conditions = Cache.create('fact-conditions-' + accountID, true, function(key, next) {
          return account.database.collection('fact_conditions').find().toArray(next);
        });
      }
    ...
    return async.series(fns, function(err, results) {
      var actions, called, conditions, context, doConditions, evals, evaluate, fact, filter, hooks, key, props, settings;
      filter = function(obj) {
      ...
      doConditions = function(condition, next) {
        ...
        evalCond = function(cond, next2) {
          return fact.data["eval"](cond, context, next2);
        };
        return async.mapSeries(condition.conditions, evalCond, function(err, result) {
