...
(function() {
  var Cache, async, evaluator, hasher, http, loadFactBase, loadFactCache, moment, q,
    __slice = [].slice;
  ...
  hasher = function() {
    return Array.prototype.join.call(arguments, '=-_,_-=');
  };

  loadFactBase = function(account) {
    return function(type, id) {
      var defer, hash, result;
      hash = hasher(account, type, id);
      if (loadFactCache[hash] != null) {
  ...
  module.exports = {
    ...
    exec: function(job, done) {
      ...
      return async.series(fns, function(err, results) {
        ...
        evaluate = function(obj, next) {
          return evaluator(fact, obj.key, obj.settings, context, next);
        };
        ...
        return async.mapSeries(evals, evaluate, function(err, cols1) {
          ...
          return async.mapSeries(conditions, doConditions, function(err, cols2) {
            var column, columns, list, mode, set, _i, _len;
